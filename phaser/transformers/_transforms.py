...
class Transformer(ABC):
    ...
    def __init__(self, name:str="", saveToDir:str='', saveToSubDir:bool=True) -> None: 
    ...
    def saveToDisk(self, image:Image, original_path:str) -> None:
        ...
        if self.saveToDir : 
            if self.saveToSubDir:
                dirpath = os.path.join(self.saveToDir, self.name)
            else:
                dirpath = self.saveToDir
            pathlib.Path(dirpath).mkdir(parents=True, exist_ok=True)
            filepath = os.path.abspath(os.path.join(dirpath, os.path.basename(original_path)))
            print(filepath)
            image.save(filepath)
...
class TransformFromDisk(Transformer):
    ...
    Args:
        Transformer (_type_): Based class inheritence, though it's not really needed in this case.
    """
...
class Border(Transformer):
    def __init__(self, border_colour:Tuple[int, int, int]=(255,255,255), border_width:int=10, saveToDir:str='', name:str="", saveToSubdir:bool=True) -> None:
        """_summary_

        Args:
            border_colour (Tuple[int, int, int], optional): _description_. Defaults to (255,255,255).
            border_width (int, optional): _description_. Defaults to 10.
            name (str, optional): Name for the transform - optional, otherwise derived from the provided arguments. Serves as directory name if saveToSubDir is set to True, otherwise can be used in graphs. Defaults to "".
            saveToDir (str, optional): Output directory if provided, if not, then the transformed image is not saved. If saveToSubDir is set then this will be the top-level directory. Defaults to ''.
            saveToSubDir (bool, optional): Requires saveToDir to be set to do anything - creates a subdirectory based on the name of the transform. Defaults to False.
        ...
        if not self.name:
                self.name = f"Border_bw{border_width}_bc{'.'.join([str(n) for n in border_colour])}"
    ...
    def fit(self, image_obj) -> Image:
        ...
        if self.saveToDir: 
            self.saveToDisk(image=image, original_path=image_obj.filename)
...
class Crop(Transformer):
    ...
    """
    def __init__(self, cropbox_factors:List[float]=[], cropbox_absolute:List[int]=[], name:str="", saveToDir:str="",  saveToSubdir:bool=True) -> None:
        ...
        if cropbox_factors and cropbox_absolute:
            raise Exception("Transformer.Rescale requires either cropbox_factors or cropbox_absolute, not both.")
...
class Enhance(Transformer):
    def __init__(self, brightnessfactor:float=1.0, colourfactor:float=1.0, contrastfactor:float=1.0, sharpnessfactor:float=1.0, name:str="", saveToDir:str="",  saveToSubdir:bool=True) -> None:
    ...
    def fit(self, image_obj) -> Image:
        ...
        if self.colourfactor != 1.0:
            current_colour = ImageEnhance.colour(image)
...
class Flip(Transformer):
    def __init__(self, direction="horizontal", name:str="", saveToDir:str="",  saveToSubdir:bool=True) -> None:
...
class Rescale(Transformer):
    def __init__(self, scalefactor:float=None, fixed_dimensions:Tuple[int, int]=None, thumbnail_aspect:bool=True, name:str="", saveToDir:str="",  saveToSubdir:bool=True) -> None:
        ...
        if scalefactor and fixed_dimensions:
            raise Exception("Transformer.Rescale requires either scalefactor or fixed_dimensions, not both.")
        elif scalefactor:
    ...
    def fit(self, image_obj) -> Image:
        ...
        if self.scalefactor:
            ...
            self.dimensions = (int(image.width * self.scalefactor), int(image.height * self.scalefactor))
...
class Rotate(Transformer):
    def __init__(self, degrees_counter_clockwise:int=5, name:str="", saveToDir:str="",  saveToSubdir:bool=True) -> None:
        """_summary_ Must call the fit function to apply the transform and return a PIL.Image.
...
class Watermark(Transformer):
    def __init__(self, image_height_factor:float=0.1, minheight:int=40, watermark_path:str=r"../resources/watermark.png", name:str="", saveToDir:str="",  saveToSubdir:bool=True) -> None:
        """_summary_ Must call the fit function to apply the transform and return a PIL.Image.
        ...
        if watermark_path == r"../resources/watermark.png":
            ...
            self.watermark_path = os.path.join(package_dir,r'../resources/watermark.png')
    ...
    def fit(self, image_obj) -> Image:
        ...
        self.watermark_im.convert("RGBA")
        image.paste(self.watermark_im, (image.width - targetwidth, image.height - targetheight), self.watermark_im)
